...
- Nessun invio automatico di messaggi redatti da un LLM **verso host o prospect**: tutto passa da `approvals`. Eccezione: le email transazionali con testo fisso verso chi ha avviato un flusso con noi (conferma poster, depliant di benvenuto, correzione codice) partono dirette, anche se differite nel tempo. Il criterio non è la vicinanza temporale ma l'assenza di giudizio: se il testo è deciso in anticipo e uguale per tutti, non c'è nulla da approvare. Serve approvals quando il contenuto è generato da un LLM o dipende dal caso specifico. Restano comunque loggate in `messages` prima dell'invio. Fuori perimetro: i messaggi che Argo manda a Leonardo (es. modo "orienta", cantiere Argo — la voce) non passano da qui — stessa logica dell'eccezione su `messages` sopra, Leonardo è il destinatario e insieme l'unico che potrebbe approvare, un'approvazione sarebbe circolare. `approvals` resta obbligatorio per tutto ciò che esce verso un host o un prospect.
